...
image_colorize = ImageOps.colorize(image=image.convert('L'), black='black', white='red')

# dimension changes
image_mirror = ImageOps.mirror(image)
image_flip = ImageOps.flip(image)
image_scale = ImageOps.scale(image=image, factor=2)
...
image_fit = ImageOps.fit(image=image, size=(300,300))
image_crop = ImageOps.crop(image=image, border=250)
